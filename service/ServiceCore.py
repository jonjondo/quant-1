...
class WhiteGuardStockCore:
    ...
    def get_stock_dmi_my_signal(self,stock_id,start_day,end_day):
        N,MM=14,6
        ret, stock_ret = self.quote_ctx.get_history_kline(stock_id, start_day,end_day, ktype='K_DAY', autype='qfq')  # 获取历史K线
        if not stock_ret.empty:
            dfret = self.get_stock_dmi_my_signal_data(stock_ret)
            ...
            #plt.xticks(pd.date_range(start_day,end_day),rotation=90)
            ...
            plt.grid()
            #记得加这一句，不然不会显示图像
            plt.savefig('static/'+stock_id+".png")
