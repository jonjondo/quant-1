...
class AutoFutuTrade:
    ...
    def auto_trade(self,stock_id):
        ...
        if df.iat[-1,-1] < 0 and df.iat[-2,-1] < df.iat[-1,-1] and df.iat[-2,-1] == minaaj:
        #if test_mode == True:
    ...
    def make_sell_order(self,unlock_password,trade_env,order_side):
        ...
        for i in range(0, len(df)):
            if int(df.iloc[i]['can_sell_qty']) > 0:
                ...
                if df_dmi.iat[-1,-1] > 0 and df_dmi.iat[-2,-1] > df_dmi.iat[-1,-1] and df_dmi.iat[-2,-1] == maxaaj: #AAJ大于多少卖出,且算AJJ是不是出了顶部拐点。
