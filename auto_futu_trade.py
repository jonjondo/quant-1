...
class AutoFutuTrade:
    def __init__(self):
        ...
        self.wgs= WhiteGuardStockCore('192.168.0.105',11111)
    ...
    def auto_trade(self):
        #print("hello %s" % name)
        ret,self.df_snapshot = self.wgs.get_current_trade_list_info(TRADE_LIST) #想获取最新快照，传每手的参数进去，但好像感觉不行，值是对的，成交时候参数错误
    ...
    def make_buy_order(self,unlock_password, stock_id, trade_env,order_side,lot_size,qty):
        return self.wgs.open_trade_make_order(unlock_password, stock_id, trade_env,order_side,lot_size,qty) #qty买入的时候qty填金额，比如买多少钱为上限，卖出的时候传卖几手
